...
import com.wiserock.heruri.databinding.FragmentPlanner1Binding
import com.wiserock.heruri.utils.classes.Planner
import com.wiserock.heruri.view.adapter.PlannerAdapter
import kotlinx.android.synthetic.main.fragment_planner1.view.*
import java.util.*

class Planner1Fragment : Fragment() {
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val binding: FragmentPlanner1Binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_planner1, container, false)
        val viewModel = ViewModelProvider(this).get(PlannerViewModel::class.java)
        PlannerAdapter.viewModel = viewModel
        val view = binding.root
        val recycler = view.fragment1_planner_recycler
        recycler.layoutManager = StaggeredGridLayoutManager(7, 1)
        Planner.selectedCalendar.set(Calendar.MONTH, -1)
